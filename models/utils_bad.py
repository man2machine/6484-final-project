...
from models.d4rl_infos import REF_MIN_SCORE, REF_MAX_SCORE, D4RL_DATASET_STATS
...
def evaluate_on_env_s4(model, device, context_len, env, rtg_target, rtg_scale,
                    sparse_reward=False, num_eval_ep=10, max_test_ep_len=1000,
    ...
    with torch.no_grad():
        ...
        for _ in range(num_eval_ep):
            ...
            for t in range(max_test_ep_len):
                ...
                if t < context_len:
                    combined_trs = torch.cat((states[:,:context_len]))
...
class D4RLTrajectoryDataset(Dataset):
    def __init__(self, dataset_path, context_len, rtg_scale, rtg_sparse_flag):
...
class S4D4RLTrajectoryDataset(D4RLTrajectoryDataset):
    ...
    def __getitem__(self, idx):
        ...
        states = torch.from_numpy(traj['observations'][si : si + self.context_len])
        actions = torch.from_numpy(traj['actions'][si : si + self.context_len])
        combined_trs = torch.cat((timesteps, returns_to_go, states), dim=-1)
        # print(combined_rsa.size())
        return combined_trs, actions
